    ...
    public async Task Update(TgChat chat, UploadedDataState state, DateTime timeStamp)
    {
        switch (state)
        {
            case UploadedDataState.New:
                await ProcessNew(chat, timeStamp);
                break;
        ...
        private async Task ProcessExisting(TgChat chat, DateTime timeStamp)
        {
        ...
        private Task ProcessNew(TgChat chat, DateTime timeStamp) 
